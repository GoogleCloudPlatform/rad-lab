...
resource "google_project_iam_member" "role_data_ingest_viewer" {
...
resource "google_project_iam_member" "role_data_govern_viewer" {
  for_each = toset(concat(formatlist("user:%s", var.trusted_users), formatlist("group:%s", var.trusted_groups)))
  member   = each.value
  project  = module.project_radlab_sdw_data_govern.project_id
  role     = "roles/viewer"
}
resource "google_project_iam_member" "role_conf_data_viewer" {
  for_each = toset(concat(formatlist("user:%s", var.trusted_users), formatlist("group:%s", var.trusted_groups)))
  member   = each.value
  project  = module.project_radlab_sdw_conf_data.project_id
  role     = "roles/viewer"
}
resource "google_project_iam_member" "role_non_conf_viewer" {
  for_each = toset(concat(formatlist("user:%s", var.trusted_users), formatlist("group:%s", var.trusted_groups)))
  member   = each.value
...
resource "google_project_iam_member" "role_data_ingest_owner" {
  for_each = toset(concat(formatlist("user:%s", var.owner_users), formatlist("group:%s", var.owner_groups)))
  member   = each.value
  project  = module.project_radlab_sdw_data_ingest.project_id
  role     = "roles/owner"
}
resource "google_project_iam_member" "role_data_govern_owner" {
  for_each = toset(concat(formatlist("user:%s", var.owner_users), formatlist("group:%s", var.owner_groups)))
  member   = each.value
  project  = module.project_radlab_sdw_data_govern.project_id
  role     = "roles/owner"
