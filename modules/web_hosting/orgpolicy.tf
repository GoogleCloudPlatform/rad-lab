...
resource "google_project_organization_policy" "bucket_level_access_policy" {
  count      = var.set_bucket_level_access_policy ? 1 : 0
...
resource "time_sleep" "wait_120_seconds" {
  count = var.set_shielded_vm_policy || var.set_bucket_level_access_policy || (var.set_domain_restricted_sharing_policy && var.create_budget && var.billing_budget_pubsub_topic) || var.enable_services ? 1 : 0
  depends_on = [
    google_project_organization_policy.bucket_level_access_policy,
