...
resource "null_resource" "build_and_push_image" {
  ...
  provisioner "local-exec" {
    ...
    command     = "scripts/build/build.sh ${local.project.project_id} ${var.zone} ${var.image_name} ${google_artifact_registry_repository.containers_repo.location}-docker.pkg.dev/${local.project.project_id}/${google_artifact_registry_repository.containers_repo.repository_id}/${var.image_name} ${google_storage_bucket.notebooks_bucket.name} ${replace(local.network.self_link, "https://www.googleapis.com/compute/v1/", "")}"
  }
